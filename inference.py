...
def main():
    ...
    parser.add_argument('--ckpt_dir',type=str, default="checkpoints/lora/infographic")
    ...
    args.output_dir=osp.join(output_base_dir,args.output_dir)
    ...
    if config.byt5_ckpt_dir is not None:
        byt5_state_dict = torch.load(osp.join(config.byt5_ckpt_dir, BYT5_BASE_CKPT_NAME), map_location='cpu')
    ...
    for name, module in unet.named_modules():
        if isinstance(module, BasicTransformerBlock):
            ...
            module.forward = types.MethodType(monkey_patch_region_attention_glyph_attention_select, module)
    ...
    try:
        ...
        byt5_mapper_para = torch.load(osp.join(config.byt5_mapper_path, BYT5_MAPPER_CKPT_NAME), map_location='cpu')
        byt5_mapper.load_state_dict(byt5_mapper_para)
        print(f"loaded byt5_mapper from {config.byt5_mapper_path}")
    # byt5
    byt5_model_para = torch.load(osp.join(config.byt5_ckpt_dir, BYT5_CKPT_NAME), map_location='cpu')
    ...
    # run inference
    generator = torch.Generator(device=args.device).manual_seed(config.seed) if config.seed else None

    args.width=int(args.width//32*32)
    args.height=int(args.height//32*32)

    for idx, region_meta_item in tqdm(enumerate(sample_list), total=len(sample_list)):
        ...
        for item in region_meta:
            item["top_left"][0]=int(item["top_left"][0]/orig_width*args.width)
            item["top_left"][1]=int(item["top_left"][1]/orig_height*args.height)
            item["bottom_right"][0]=int(item["bottom_right"][0]/orig_width*args.width)
            item["bottom_right"][1]=int(item["bottom_right"][1]/orig_height*args.height)
        ...
        try:
            ...
            #draw the lcfg map
            draw_lcfg(index,region_meta,dir=args.output_dir,guidance=args.guidance_scale)
